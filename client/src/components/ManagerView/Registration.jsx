...
import { ManagerIdContext, PageContext } from "./ManagerView";
import axios from "axios";
import "./styles/Registration.css";
import { Translate } from "../Translation/TranslationWrapper";

function Registration() {
  const { setManagerId } = useContext(ManagerIdContext);
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/manager/login`,
        {
      ...
      if (response.data.manager_id) {
        ...
        navigate("/manager/statistics"); // Redirect after successful login
      }
    } catch (err) {
      if (err.response) {
      ...
      <div className="registration">
        <div className="red-container" id="registration-red-container">
          <h1 className="subTitle" id="login-title">
            <Translate>Log in to manage</Translate>
          </h1>
          <input
